subroutine ReadInputFile
  ...
  use module_mosaic_sect_intr, only:  &!BSINGH - 05/28/2013(RCE updates)
       sect_intr_allocate_memory
  implicit none
...
! read(lun_inp,*)RH                     ! relative humidity [%]
  eword = ' '
  do while ( eword == ' ' )
     read(lun_inp,'(a)') eword
  end do
  idum = 0
  do i = 10, 67
! if eword contains 'rh and rh_updn', then read rh and rh_updn_* values
     if (eword(i:i+13) == 'rh and rh_updn') then
        read(eword,*,iostat=k) rh, rh_updn_delta, rh_updn_loval, rh_updn_hival
        if (k == 0) idum = 1
        exit
     end if
  end do
  if (idum == 0) then
! otherwise just read rh and set the rh_updn_* to 'do nothing' values
     rh_updn_delta = 0.0 ; rh_updn_loval = 0.0 ; rh_updn_hival = 100.0
     read(eword,*) RH                     ! relative humidity [%]
  end if
  ...
  if (m_partmc_mosaic <= 0) call sect_intr_allocate_memory
  ...
  read(lun_inp,*) iwrite_fullout, iwrite_gas, &
     iwrite_aer_bin, iwrite_aer_dist, iwrite_aer_species, iwrite_aer_optic
  read(lun_inp,*) &
     iwrite_sect_170, iwrite_sect_171, iwrite_sect_172, iwrite_sect_190 
  read(lun_inp,*) &
     iwrite_sect_180, iwrite_sect_183, iwrite_sect_184, &
     iwrite_sect_185, iwrite_sect_186, iwrite_sect_188
  read(lun_inp,*) idiag_sect_coag, idiag_sect_movesect, idiag_sect_newnuc
