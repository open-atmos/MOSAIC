subroutine SetIOfiles
  ...
  use module_data_mosaic_main,   only: maer, maeroptic, mgas
  use module_data_mosaic_boxmod,   only: &
       iwrite_aer_bin, iwrite_aer_dist, iwrite_aer_species,       &
       iwrite_aer_optic, iwrite_fullout, iwrite_gas,   &
       iwrite_sect_170, iwrite_sect_171, iwrite_sect_172, iwrite_sect_180, &
       iwrite_sect_183, iwrite_sect_184, iwrite_sect_185, iwrite_sect_186, &
       iwrite_sect_188, iwrite_sect_190, &
       lun_sect_170, lun_sect_171, lun_sect_172, lun_sect_180, &
       lun_sect_183, lun_sect_184, lun_sect_185, lun_sect_186, &
       lun_sect_188, lun_sect_190,    &
       inputfile, lun_inp, lun_gas, lun_drydist, lun_wetdist,     &
       lun_species, lun_aeroptic, lun_aer_status, lun_aer,        &
       lun_fullout,                                               &
       gas_output, drydist_output,    &
       wetdist_output, species_output, aeroptic_output, &
  ...
  use module_data_mosaic_pmcmos, only: lun_pmcmos_in1,            &
       lun_pmcmos_in2
  use module_data_mosaic_asecthp,  only: lunout
  ...
  if (iwrite_sect_170 <= 0) lun_sect_170 = 0
  if (iwrite_sect_171 <= 0) lun_sect_171 = 0
  if (iwrite_sect_172 <= 0) lun_sect_172 = 0
  if (iwrite_sect_180 <= 0) lun_sect_180 = 0
  if (iwrite_sect_183 <= 0) lun_sect_183 = 0
  if (iwrite_sect_184 <= 0) lun_sect_184 = 0
  if (iwrite_sect_185 <= 0) lun_sect_185 = 0
  if (iwrite_sect_186 <= 0) lun_sect_186 = 0
  if (iwrite_sect_188 <= 0) lun_sect_188 = 0
  if (iwrite_sect_190 <= 0) lun_sect_190 = 0
