subroutine time_integration_mode( gam_ratio, iter_MESA, aH2O_a)
  ...
  use module_data_mosaic_main,  only: &
       ipmcmos, mgas, maer, maeroptic, msolar, ngas_max
  use module_data_mosaic_boxmod,  only: dt_min, dt_aeroptic_min,&
       nstep, iprint, iwrite_sect_190, lun_sect_190, &
       pblh, rh, pr_atm, te, time_sec, cair_mlc, ppb, &
       cnn
  ...
  if ((maeroptic > 0) .and. (maer == myes)) then
     ...
     !RCE - When aerosol_optical gets fixed, define cair_mol_m3 = cair_molm3
