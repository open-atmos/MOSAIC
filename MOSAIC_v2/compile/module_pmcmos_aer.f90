...
!		write(*,'(a,i5,1p,2e11.3)') 'l,tmp_m,v =', l, tmp_m, tmp_v
		...
		tmp_dlo = dlo_sect(isize,itype)
		tmp_dhi = dhi_sect(isize,itype)
		call modal_to_bin_aa( ad%dgnum_cm(imode), ad%lnsg(imode), &
		    tmp_dlo, tmp_dhi, fracnum, fracvol )
...
!
	...
	use module_data_mosaic_boxmod, only:  &
	    cair_mlc, cair_molm3, cnn,  &
            inputfile, iprint, iwrite_fullout,  &
	    lun_sect_183, lun_sect_184, lun_sect_185, lun_sect_186,  &
	    lun_sect_188, lun_fullout,  &
	    ppb, pr_atm, rh, species, te, time_sec
	use module_data_mosaic_aero, only:  &
	    aer_name, gas_name, &
	    method_bcfrac, method_kappa, &
...
!   local variables
	...
	integer  :: ioptaa_fullout, ioptbb_fullout
	integer  :: j, jj
	integer  :: l, ltmp, lun
	...
	type( aer_dist_t ) :: ad
        ...
        ! the module_data_mosaic_asecthp arrays, which are used in this routine,
...
!	if (time_sec <= 1.0e-10) then
	if (it == 0 .and. lun_sect_183 > 0) then
	...
	lun = lun_sect_188
	if (msectional_flag2 > 0 .and. lun > 0) then
...
! fullout output
	...
	do l = 1, ngas_max
	    ...
	    if (ioptbb_fullout == 168) then
	    write(lun,'(i8,1p,e16.8,2x,a)') it, cnn(l)*tmpa, species(l)(1:12)
	    else
