...
!        Keq_nh4cl_0  = min(Kp_nh4cl_0,  Keq_sg(2))	! 6/25/2008, RAZ 9/10/2020
	Keq_nh4cl_0 = kel(inh3_g,ibin)*kel(ihcl_g,ibin)*Keq_sg(2)	! RAZ 9/10/2020
...
!      Keq_nh4no3_0 = min(Kp_nh4no3_0, Keq_sg(1))	! 6/25/2008, 9/10/2020
!      Keq_nh4cl_0  = min(Kp_nh4cl_0,  Keq_sg(2))	! 6/25/2008, 9/10/2020

      Keq_nh4no3_0 = kel(inh3_g,ibin)*kel(ihno3_g,ibin)*Keq_sg(1)	! RAZ 9/10/2020
      Keq_nh4cl_0  = kel(inh3_g,ibin)*kel(ihcl_g,ibin)*Keq_sg(2)	! RAZ 9/10/2020
...
!      phi_nh4cl_s = (gas(inh3_g)*gas(ihcl_g) - Keq_sg(2))/   &	! RAZ 9/10/2020
!                    max(gas(inh3_g)*gas(ihcl_g),Keq_sg(2))

      phi_nh4cl_s = (gas(inh3_g)*gas(ihcl_g) - Keq_nh4cl_0)/ &	! RAZ 9/10/2020
		     max(gas(inh3_g)*gas(ihcl_g),Keq_nh4cl_0)
...
! compute diagnostic products and ratios
      phi_nh4no3_s = (gas(inh3_g)*gas(ihno3_g) - Keq_nh4no3_0)/   &	! 9/10/2020
                     max(gas(inh3_g)*gas(ihno3_g),Keq_nh4no3_0)		! 9/10/2020
      phi_nh4cl_s  = (gas(inh3_g)*gas(ihcl_g) - Keq_nh4cl_0)/   &	! 9/10/2020
                     max(gas(inh3_g)*gas(ihcl_g),Keq_nh4cl_0)		! 9/10/2020
...
! nh4no3
      if( abs(phi_nh4no3_s) .lt. rtol_eqb_ASTEM )then
        iactive_nh4no3 = 0
      elseif(gas(inh3_g)*gas(ihno3_g) .lt. Keq_nh4no3_0 .and.   &	! 9/10/2020
...
! nh4cl
      if( abs(phi_nh4cl_s) .lt. rtol_eqb_ASTEM )then
        iactive_nh4cl = 0
      elseif(gas(inh3_g)*gas(ihcl_g) .lt. Keq_nh4cl_0 .and.   &		! 9/10/2020
             epercent(jnh4cl, jsolid,ibin) .le. ptol_mol_ASTEM)then
        iactive_nh4cl = 0
        if(epercent(jnh4cl, jsolid,ibin) .gt. 0.0)then
          call degas_solid_nh4cl(ibin)
...
! reset
	Keq_nh4no3_0 = kel(inh3_g,ibin)*kel(ihno3_g,ibin)*Keq_sg(1)	! 6/25/2008, 9/10/2020
	Keq_nh4cl_0  = kel(inh3_g,ibin)*kel(ihcl_g,ibin)*Keq_sg(2)	! 6/25/2008, 9/10/2020
...
! MIXED CASE 3: hno3 and hcl exchange may happen here and nh4cl may form/evaporate
      ...
      if(volatile_cl .gt. 0.0 .and. gas(ihno3_g).gt. 0.0 )then
        ...
        prod_nh4cl = max( (gas(inh3_g)*gas(ihcl_g)-Keq_nh4cl_0), 0.0d0) +   &	! 9/10/2020
...
! nh4no3 (solid)
      phi_nh4no3_s = (gas(inh3_g)*gas(ihno3_g) - Keq_nh4no3_0)/   &	! 9/10/2020
                     max(gas(inh3_g)*gas(ihno3_g),Keq_nh4no3_0)
