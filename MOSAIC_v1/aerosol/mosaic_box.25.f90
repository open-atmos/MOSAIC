...
!-----------------------------------------------------------------------
      ...
      use module_data_mosaic_main, only:  pi		! RAZ 9/10/2020 added here
      use module_data_mosaic_aero
...
! new wet mass and wet volume
      mass_wet_a(ibin) = mass_dry_a(ibin) + water_a(ibin)*1.e-3		! g/cc(air)
      vol_wet_a(ibin)  = vol_dry_a(ibin) + water_a(ibin)*1.e-3		! cc(aer)/cc(air) or m^3/m^3(air)
      Dp_wet_a(ibin)=(6.*vol_wet_a(ibin)/(num_a(ibin)*pi))**(1./3.)	! [cm]		RAZ 9/10/2020

      call calculate_kelvin(ibin)
!      kelvin(ibin) = 1.0

      if(jaerosolstate(ibin) .eq. all_solid)goto 20	! RAZ 9/10/2020
...
20    do iv = 1,  ngas_volatile
        term = 4.*sigma_soln(ibin)*partial_molar_vol(iv)/   &
                       (8.3144e7*T_K*Dp_wet_a(ibin))		! RAZ 9/10/2020 (replaced DpmV by Dp_wet_a)
...
! local variables
      integer je
      real(r8) :: term, sum_dum, slope_sigma, const_sigma, sigma_flat
      real(r8) :: moles_water, moles_amso4, moles_amno3, moles_total, &
                  x_water, x_amso4, x_amno3, &
                  Fw_amso4, Fw_amno3, Famso4_w, Famno3_w, &
                  sigma_amso4, sigma_amno3, lnsigma
...
!      sigma_soln(ibin) = sigma_water + 4.9*(1. - aH2O_a(ibin)) 	! [dyn/cm]	! Bug fix: RAZ 10/30/2019


      if(water_a(ibin) .gt. 0.0)then
! Basis: 1 m^3
      moles_water = water_a(ibin)*1000./18.		! moles
      moles_amso4 = aer(iso4_a,jliquid,ibin)*1.e-9	! moles
      moles_amno3 = aer(ino3_a,jliquid,ibin)*1.e-9	! moles
      moles_total = moles_water + moles_amso4 + moles_amno3
      x_water = moles_water/moles_total
      x_amso4 = moles_amso4/moles_total
      x_amno3 = moles_amno3/moles_total
      Fw_amso4 = 0.0 + 0.366*T_K
      Fw_amno3 = -24.245 + 0.14*T_K
      Famso4_w = 0.0 - 0.289*T_K
      Famno3_w = 0.0

      sigma_amso4 = 228.5 - 0.146*T_K	! dyn/cm
      sigma_amno3 = 148.4 - 0.105*T_K	! dyn/cm

      lnsigma = x_water*log(sigma_water + x_amso4*Fw_amso4 + x_amno3*Fw_amno3) + &
                x_amso4*log(sigma_amso4 + x_water*Famso4_w) + &
                x_amno3*log(sigma_amno3 + x_water*Famno3_w)

      sigma_flat  = exp(lnsigma)
      sigma_soln(ibin) = 30. ! sigma_flat ! exp(lnsigma)

      else

      sigma_soln(ibin) = 500.	! for solid aerosols

	write(6,*)"solid phase", ibin

      endif


      term = 4.*18.*sigma_soln(ibin)/(8.3144e7*T_K*Dp_wet_a(ibin))	! [-] RAZ 9/10/2020 (replaced DpmV by Dp_wet_a)
...
! surface tension of pure water in dyn/cm
      if(T_K .ge. 273.15d0)then
        term = (647.15d0 - T_K)/647.15d0
        sigma_water = 235.8d0*term**1.256d0 * (1.d0 - 0.625d0*term) ! dyn/cm	9/9/2020
      elseif(T_K .lt. 273.15d0)then
	sigma_water = 75.65085d0 - 0.13350d0*(T_K - 273.15d0) + 0.0019462d0*(T_K - 273.15d0)**2.d0	! dyn/cm	9/9/2020
      endif
...
!      enddo
      ...
      do je = 1, 19
        molality0(je) = bin_molality(je,ibin)	! compute aH2O dependent binary molalities RAZ 9/10/2020
      enddo
